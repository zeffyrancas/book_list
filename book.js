...
const $harf_explanation        = document.getElementById('harf_explanation');
const $add_harf_box            = document.getElementById('add_harf_box');
const $add_harf_ok             = document.getElementById('add_harf_ok');
const $add_harf_clear          = document.getElementById('add_harf_clear');
...
$add_harf_ok                .addEventListener('click',add_harf_ok);
$add_harf_clear             .addEventListener('click',add_harf_clear);
...
function list_btn(index){
    ...
    if(type == "nobel"){
        ...
        const harf_btn = document.getElementById("add_harf")
        if(harf_btn == null){
            add_harf_btn()
        }
    }
...
function register(){
    ...
    const index = sessionStorage.getItem('index');
    const number = Math.floor(sessionStorage.getItem('number'));
    const type = sessionStorage.getItem('type');
...
// ノベルのみ「.5巻」追加ボタンを付与
function add_harf_btn(){
    const btn = document.createElement('button');
    const br = document.createElement('br');
    btn.textContent = "「.5巻」追加";
    btn.setAttribute('id','add_harf');
    btn.setAttribute('class','sub_btn');
    btn.addEventListener('click',add_harf);
    $completed_btn.insertAdjacentElement('beforebegin',btn);
    $completed_btn.insertAdjacentElement('beforebegin',br);
...
function add_harf(){
    ...
    const number = sessionStorage.getItem('number');
    $add_edit.classList.add("hidden");
    $add_edit_top.classList.add("hidden")
    $add_harf_box.classList.remove("hidden");
    const p = document.createElement('p');
    p.textContent = "「.5巻を挿入する箇所を選択してください";
    p.setAttribute("id","harf_ex")
    p.setAttribute('class','sub_menu');
    $harf_explanation.appendChild(p);

    for(let i = 1; i < number; i++){
        const check_id = "check_"+i;
        const check_btn = document.getElementById(check_id);
        if(nobel_list[index][i+0.5] == undefined){
            const point = document.createElement("input");
            const point_id = "point_" + i;
            point.type = 'checkbox';
            point.setAttribute("id",point_id);
            point.setAttribute("class","check_btn")
            check_btn.insertAdjacentElement("afterend",point)
            
        }else{
            const check_id_harf = "check_"+i + ".5";
            const check_btn_harf = document.getElementById(check_id_harf);
            check_btn_harf.classList.add("hidden")
        }
        check_btn.classList.add("hidden")
    }

}

// 「.5巻」：決定
function add_harf_ok(){
    const index = sessionStorage.getItem('index');
    const number = sessionStorage.getItem('number');
    ...
    for(let i = 1; i < number; i++){
        const harf = i + 0.5;
        const point_id = "point_" + i;
        const point =  document.getElementById(point_id);
        if(point != null && point.checked){
            nobel_list[index][harf] = "YET_READ";
        }
    }
    localStorage.setItem('nobel_list',JSON.stringify(nobel_list));
    add_harf_clear()
...
// 「.5巻」：やめる
function add_harf_clear(){
    const index = sessionStorage.getItem('index');
    const number = sessionStorage.getItem('number');
    const p = document.getElementById("harf_ex");
    p.remove();

    $add_edit_top.classList.remove("hidden")
    $add_harf_box.classList.add("hidden");
    ...
    for(let i = 1; i < number; i++){
        ...
        check_btn.classList.remove("hidden")
    }
    list_btn(index);
}
